...
fun render_sidebar_html(sidebar, current_id, base_path)
    ...
    let template = io.read("templates/sidebar.html")
...
fun render_doc(title, content, sidebar_html, css_path, edit_url)
    let template = io.read("templates/doc.html")
...
fun render_index_redirect()
    let template = io.read("templates/redirect.html")
...
fun build()
    ...
    puts("Reading from docs/")

    # Find all markdown files in docs/
    let md_files = io.glob("docs/**/*.md")
    let docs = []
    ...
    while i < md_files.len()
        ...
        # Extract doc ID from path (relative to docs/)
        let relative_path = file_path.slice(5, file_path.len())  # Remove "docs/"
        ...
        let github_base = "https://github.com/lolsborn/quest/blob/main/docs/docs/"
        ...
        # Write output file
        let output_file = "output/" .. doc_id .. ".html"

        # Create subdirectories if needed
        if path_parts.len() > 1
            let dir_path = "output/" .. path_parts[0]
    ...
    io.write("output/index.html", index_html)

    # Copy static assets
    puts("Copying static assets...")

    # Create directories if they don't exist
    if not io.exists("output/css")
        os.mkdir("output/css")
    end
    if not io.exists("output/js")
        os.mkdir("output/js")
    end

    let css_files = io.glob("public/css/*.css")
    let k = 0
    while k < css_files.len()
        let filename = css_files[k].split("/").last()
        let content = io.read(css_files[k])
        io.write("output/css/" .. filename, content)
        k = k + 1
    end

    let js_files = io.glob("public/js/*.js")
    let m = 0
    while m < js_files.len()
        let filename = js_files[m].split("/").last()
        let content = io.read(js_files[m])
        io.write("output/js/" .. filename, content)
        m = m + 1
    end

    # Generate search index
    let search_index_json = json.stringify(docs)
    io.write("output/search-index.json", search_index_json)
