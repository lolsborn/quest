...
test.describe("Literals", fun ()
    ...
    test.it("supports negative literals", fun ()
        let x = -123n
        test.assert_eq(x.to_string(), "-123", nil)
    end)
end)
...
test.describe("Construction", fun ()
    test.it("creates BigInt from string", fun ()
        ...
        test.assert_eq(x.to_string(), "999999999999999999999999999999", nil)
    end)

    test.it("creates BigInt from int", fun ()
        let x = bigint.from_int(42)
        test.assert_eq(x.to_string(), "42", nil)
    end)

    test.it("creates BigInt from negative string", fun ()
        let x = bigint.new("-123456789012345678901234567890")
        test.assert_eq(x.to_string(), "-123456789012345678901234567890", nil)
    end)
...
test.describe("Constants", fun ()
    test.it("has ZERO constant", fun ()
        test.assert_eq(bigint.ZERO.to_string(), "0", nil)
    end)

    test.it("has ONE constant", fun ()
        test.assert_eq(bigint.ONE.to_string(), "1", nil)
    end)

    test.it("has TWO constant", fun ()
        test.assert_eq(bigint.TWO.to_string(), "2", nil)
    end)

    test.it("has TEN constant", fun ()
        test.assert_eq(bigint.TEN.to_string(), "10", nil)
    end)
end)
...
test.describe("Arithmetic - Addition", fun ()
    test.it("adds two BigInts with literals", fun ()
        ...
        let b = 1n
        let c = a.plus(b)
        test.assert_eq(c.to_string(), "100000000000000000000", nil)
    end)
    ...
    test.it("adds large BigInts", fun ()
        ...
        let c = a.plus(b)
        test.assert_eq(c.to_string(), "111111111011111111101111111110111111111011111111100", nil)
    end)
end)
...
test.describe("Arithmetic - Subtraction", fun ()
    test.it("subtracts BigInts", fun ()
        ...
        let b = bigint.new("1")
        let c = a.minus(b)
        test.assert_eq(c.to_string(), "99999999999999999999", nil)
    end)
    ...
    test.it("handles negative results", fun ()
        ...
        let b = bigint.new("10")
        let c = a.minus(b)
        test.assert_eq(c.to_string(), "-5", nil)
    end)
end)
...
test.describe("Arithmetic - Multiplication", fun ()
    test.it("multiplies BigInts", fun ()
        ...
        let c = a.times(b)
        test.assert_eq(c.to_string(), "1219326311370217952237463801111263526900", nil)
    end)
end)
...
test.describe("Arithmetic - Division", fun ()
    test.it("divides BigInts", fun ()
        ...
        let b = bigint.new("10")
        let c = a.div(b)
        test.assert_eq(c.to_string(), "10000000000000000000", nil)
    end)
    ...
    test.it("truncates toward zero", fun ()
        ...
        let b = bigint.new("5")
        let c = a.div(b)
        test.assert_eq(c.to_string(), "3", nil)
    end)
...
test.describe("Arithmetic - Modulo", fun ()
    test.it("computes modulo", fun ()
        ...
        let b = bigint.new("5")
        let c = a.mod(b)
        test.assert_eq(c.to_string(), "2", nil)
    end)
...
test.describe("Arithmetic - divmod", fun ()
    test.it("returns quotient and remainder", fun ()
        ...
        test.assert_eq(result.get(0).to_string(), "3", nil)
        test.assert_eq(result.get(1).to_string(), "2", nil)
    end)
end)

test.describe("Arithmetic - Power", fun ()
    test.it("computes power", fun ()
        let x = bigint.TWO.pow(bigint.from_int(10))
        test.assert_eq(x.to_string(), "1024", nil)
    end)

    test.it("computes large power", fun ()
        let x = bigint.TWO.pow(bigint.from_int(100))
        test.assert_eq(x.to_string(), "1267650600228229401496703205376", nil)
    end)
    ...
    test.it("computes modular exponentiation", fun ()
        ...
        test.assert_eq(result.to_string(), "67077", nil)
    end)
end)
...
test.describe("Arithmetic - abs and negate", fun ()
    test.it("computes absolute value", fun ()
        ...
        let y = x.abs()
        test.assert_eq(y.to_string(), "12345678901234567890", nil)
    end)
    ...
    test.it("double negation returns original", fun ()
        ...
        test.assert_eq(y.to_string(), "42", nil)
    end)
end)
...
test.describe("Comparison", fun ()
    test.it("tests equality", fun ()
        ...
        test.assert_eq(a.equals(b), true, nil)
    end)

    test.it("tests inequality", fun ()
        let a = bigint.new("123")
        let b = bigint.new("456")
        test.assert_eq(a.not_equals(b), true, nil)
    end)

    test.it("tests less than", fun ()
        let a = bigint.new("123")
        let b = bigint.new("456")
        test.assert_eq(a.less_than(b), true, nil)
        test.assert_eq(b.less_than(a), false, nil)
    end)
    ...
    test.it("tests less than or equal", fun ()
        ...
        test.assert_eq(a.less_equal(b), true, nil)
        test.assert_eq(a.less_equal(c), true, nil)
    end)

    test.it("tests greater than", fun ()
        let a = bigint.new("456")
        let b = bigint.new("123")
        test.assert_eq(a.greater(b), true, nil)
        test.assert_eq(b.greater(a), false, nil)
    end)
    ...
    test.it("tests greater than or equal", fun ()
        ...
        let b = bigint.new("123")
        let c = bigint.new("456")
        test.assert_eq(c.greater_equal(b), true, nil)
        test.assert_eq(a.greater_equal(b), true, nil)
    end)
end)
...
test.describe("Bitwise Operations", fun ()
    test.it("performs bitwise AND", fun ()
        ...
        let c = a.bit_and(b)
        test.assert_eq(c.to_string(), "7", nil)
    end)
    ...
    test.it("performs bitwise OR", fun ()
        ...
        let c = a.bit_or(b)
        test.assert_eq(c.to_string(), "14", nil)  # 1110
    end)
    ...
    test.it("performs bitwise XOR", fun ()
        ...
        let b = bigint.new("10")  # 1010
        let c = a.bit_xor(b)
        test.assert_eq(c.to_string(), "6", nil)  # 0110
    end)

    test.it("performs bitwise NOT on zero", fun ()
        let a = bigint.new("0")
        let b = a.bit_not()
        test.assert_eq(b.to_string(), "-1", nil)
    end)
    ...
    test.it("performs bitwise NOT on non-zero", fun ()
        ...
        test.assert_eq(b.to_string(), "-6", nil)
    end)

    test.it("performs left shift", fun ()
        let a = bigint.new("1")
        let b = a.shl(10)
        test.assert_eq(b.to_string(), "1024", nil)
    end)
...
test.describe("Conversions", fun ()
    ...
    test.it("converts to float", fun ()
        ...
        let y = x.to_float()
        test.assert_eq(y, 12345.0, nil)
    end)

    test.it("converts to string with different bases", fun ()
        let x = bigint.new("255")
        test.assert_eq(x.to_string(), "255", nil)
        test.assert_eq(x.to_string(16), "ff", nil)
        test.assert_eq(x.to_string(2), "11111111", nil)
        test.assert_eq(x.to_string(8), "377", nil)
    end)
    ...
    test.it("converts to bytes", fun ()
        ...
        test.assert_eq(bytes.len(), 2, nil)
    end)
end)

test.describe("Utility Methods", fun ()
    test.it("tests is_zero", fun ()
        test.assert_eq(bigint.ZERO.is_zero(), true, nil)
        test.assert_eq(bigint.ONE.is_zero(), false, nil)
    end)

    test.it("tests is_positive", fun ()
        test.assert_eq(bigint.ONE.is_positive(), true, nil)
        test.assert_eq(bigint.ZERO.is_positive(), false, nil)
        test.assert_eq(bigint.ONE.negate().is_positive(), false, nil)
    end)

    test.it("tests is_negative", fun ()
        test.assert_eq(bigint.ONE.negate().is_negative(), true, nil)
        test.assert_eq(bigint.ZERO.is_negative(), false, nil)
        test.assert_eq(bigint.ONE.is_negative(), false, nil)
    end)

    test.it("tests is_even", fun ()
        test.assert_eq(bigint.TWO.is_even(), true, nil)
        test.assert_eq(bigint.ONE.is_even(), false, nil)
    end)

    test.it("tests is_odd", fun ()
        test.assert_eq(bigint.ONE.is_odd(), true, nil)
        test.assert_eq(bigint.TWO.is_odd(), false, nil)
    end)

    test.it("computes bit_length", fun ()
        let x = bigint.new("255")
        test.assert_eq(x.bit_length(), 8, nil)
        let y = bigint.new("256")
        test.assert_eq(y.bit_length(), 9, nil)
    end)
end)

test.describe("Large Number Operations", fun ()
    test.it("handles 2^1000", fun ()
        let x = bigint.TWO.pow(bigint.from_int(1000))
        let str = x.to_string()
        # 2^1000 has 302 digits
        test.assert_eq(str.len() > 300, true, nil)
    end)
    ...
    test.it("computes factorial(20)", fun ()
        ...
        test.assert_eq(result.to_string(), "2432902008176640000", nil)
    end)
end)
...
test.describe("Object Methods", fun ()
    test.it("has cls method", fun ()
        let x = bigint.new("42")
        test.assert_eq(x.cls(), "BigInt", nil)
    end)

    test.it("has _str method", fun ()
        let x = bigint.new("42")
        test.assert_eq(x._str(), "42", nil)
    end)

    test.it("has _rep method", fun ()
        let x = bigint.new("42")
        test.assert_eq(x._rep(), "BigInt(42)", nil)
    end)
    ...
    test.it("has _id method", fun ()
        ...
        test.assert_neq(x._id(), y._id(), nil)
    end)
end)
