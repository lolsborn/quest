...
test.describe("Constants", fun ()
    test.it("has ZERO constant", fun ()
        test.assert_eq(ZERO.to_string(), "0", nil)
    end)

    test.it("has ONE constant", fun ()
        test.assert_eq(ONE.to_string(), "1", nil)
    end)

    test.it("has TWO constant", fun ()
        test.assert_eq(TWO.to_string(), "2", nil)
    end)

    test.it("has TEN constant", fun ()
        test.assert_eq(TEN.to_string(), "10", nil)
    end)
end)
...
test.describe("Utility Methods", fun ()
    test.it("tests is_zero", fun ()
        test.assert_eq(ZERO.is_zero(), true, nil)
        test.assert_eq(ONE.is_zero(), false, nil)
    end)

    test.it("tests is_positive", fun ()
        test.assert_eq(ONE.is_positive(), true, nil)
        test.assert_eq(ZERO.is_positive(), false, nil)
        test.assert_eq(ONE.negate().is_positive(), false, nil)
    end)

    test.it("tests is_negative", fun ()
        test.assert_eq(ONE.negate().is_negative(), true, nil)
        test.assert_eq(ZERO.is_negative(), false, nil)
        test.assert_eq(ONE.is_negative(), false, nil)
    end)

    test.it("tests is_even", fun ()
        test.assert_eq(TWO.is_even(), true, nil)
        test.assert_eq(ONE.is_even(), false, nil)
    end)

    test.it("tests is_odd", fun ()
        test.assert_eq(ONE.is_odd(), true, nil)
        test.assert_eq(TWO.is_odd(), false, nil)
    end)
