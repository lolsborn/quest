...
test.describe("Array creation", fun ()
    test.it("creates zeros array", fun ()
        let m = np.zeros([3, 3])
        test.assert_eq(m.shape(), [3, 3])
        test.assert_eq(m.ndim(), 2)
        test.assert_eq(m.size(), 9)
    end)

    test.it("creates ones array", fun ()
        let m = np.ones([2, 4])
        test.assert_eq(m.shape(), [2, 4])
        test.assert_eq(m.size(), 8)
    end)

    test.it("creates full array with value", fun ()
        let m = np.full([2, 3], 5.0)
        test.assert_eq(m.shape(), [2, 3])
        test.assert_eq(m.size(), 6)
    end)
    ...
    test.it("creates 1D array", fun ()
        let v = np.zeros([5])
        test.assert_eq(v.shape(), [5])
        test.assert_eq(v.ndim(), 1)
    end)

    test.it("creates 3D array", fun ()
        let cube = np.zeros([2, 3, 4])
        test.assert_eq(cube.shape(), [2, 3, 4])
        test.assert_eq(cube.ndim(), 3)
        test.assert_eq(cube.size(), 24)
    end)
end)
...
test.describe("From nested arrays", fun ()
    ...
    test.it("creates 3D array from deeply nested arrays", fun ()
        ...
        test.assert_eq(cube.shape(), [2, 2, 2])
        test.assert_eq(cube.ndim(), 3)
        test.assert_eq(cube.size(), 8)
    end)

    test.it("handles floats in nested arrays", fun ()
        let m = np.array([[1.5, 2.5], [3.5, 4.5]])
        test.assert_eq(m.shape(), [2, 2])
    end)
end)
...
test.describe("Reshape", fun ()
    ...
    test.it("reshapes 2D to different 2D", fun ()
        ...
        test.assert_eq(m2.shape(), [3, 4])
    end)

    test.it("reshapes to 1D", fun ()
        let m = np.zeros([2, 3, 4])
        let v = m.reshape([24])
        test.assert_eq(v.shape(), [24])
        test.assert_eq(v.ndim(), 1)
    end)
...
test.describe("Transpose", fun ()
    test.it("transposes 2D array", fun ()
        let m = np.zeros([3, 4])
        let mt = m.transpose()
        test.assert_eq(mt.shape(), [4, 3])
    end)

    test.it("T alias for transpose", fun ()
        let m = np.zeros([2, 5])
        let mt = m.T()
        test.assert_eq(mt.shape(), [5, 2])
    end)
...
test.describe("Aggregations", fun ()
    ...
    test.it("sums along axis 0", fun ()
        ...
        test.assert_eq(col_sums.shape(), [4])
    end)

    test.it("sums along axis 1", fun ()
        let m = np.ones([3, 4])
        let row_sums = m.sum(1)
        test.assert_eq(row_sums.shape(), [3])
    end)
end)
...
test.describe("Type checking", fun ()
    test.it("has correct type", fun ()
        let m = np.zeros([2, 2])
        test.assert_eq(m.cls(), "NDArray")
        test.assert(m.is("NDArray"))
        test.assert(m.is("ndarray"))
    end)
