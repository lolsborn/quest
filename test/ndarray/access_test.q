...
test.describe("get() method", fun ()
    test.it("gets element from 2D array", fun ()
        ...
        test.assert_eq(m.get([0, 0]), 1.0)
        test.assert_eq(m.get([0, 2]), 3.0)
        test.assert_eq(m.get([1, 1]), 5.0)
        test.assert_eq(m.get([1, 2]), 6.0)
    end)

    test.it("gets element from 1D array", fun ()
        let v = np.array([10, 20, 30, 40])
        test.assert_eq(v.get([0]), 10.0)
        test.assert_eq(v.get([2]), 30.0)
        test.assert_eq(v.get([3]), 40.0)
    end)

    test.it("gets element from 3D array", fun ()
        let cube = np.array([[[1, 2], [3, 4]], [[5, 6], [7, 8]]])
        test.assert_eq(cube.get([0, 0, 0]), 1.0)
        test.assert_eq(cube.get([0, 1, 1]), 4.0)
        test.assert_eq(cube.get([1, 0, 1]), 6.0)
        test.assert_eq(cube.get([1, 1, 1]), 8.0)
    end)
...
test.describe("Matrix element access patterns", fun ()
    ...
    test.it("accesses diagonal elements", fun ()
        let m = np.eye(3)
        test.assert_eq(m.get([0, 0]), 1.0)
        test.assert_eq(m.get([1, 1]), 1.0)
        test.assert_eq(m.get([2, 2]), 1.0)
        test.assert_eq(m.get([0, 1]), 0.0)
    end)
end)
