...
test.describe("Timestamp Formatting", fun ()
    test.it("can format a timestamp", fun ()
        let ts = time.from_timestamp(1730318400)  # 2024-10-30 12:00:00 UTC
        let formatted = ts.format("%Y-%m-%d")
        test.assert(formatted.cls() == "Str", "format() should return a string")
        test.assert(formatted.len() > 0, "formatted string should not be empty")
    end)

    test.it("can format with time components", fun ()
        let ts = time.from_timestamp(1730318400)
        let formatted = ts.format("%Y%m%d_%H%M%S")
        test.assert(formatted.cls() == "Str", "Should return a string")
        test.assert(formatted.len() > 0, "Should not be empty")
    end)

    test.it("can use time.now().format()", fun ()
        let now = time.now()
