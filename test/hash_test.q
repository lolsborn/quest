...
test.describe("MD5", fun ()
    test.it("hashes empty string correctly", fun ()
        test.assert_eq(hash.md5(""), "d41d8cd98f00b204e9800998ecf8427e")
    end)

    test.it("hashes single character correctly", fun ()
        test.assert_eq(hash.md5("a"), "0cc175b9c0f1b6a831c399e269772661")
    end)

    test.it("hashes 'abc' correctly", fun ()
        test.assert_eq(hash.md5("abc"), "900150983cd24fb0d6963f7d28e17f72")
    end)
end)

test.describe("SHA-1", fun ()
    test.it("hashes empty string correctly", fun ()
        test.assert_eq(hash.sha1(""), "da39a3ee5e6b4b0d3255bfef95601890afd80709")
    end)
end)

test.describe("SHA-256", fun ()
    test.it("hashes empty string correctly", fun ()
        test.assert_eq(hash.sha256(""), "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855")
    end)

    test.it("hashes 'test' correctly", fun ()
        test.assert_eq(hash.sha256("test"), "9f86d081884c7d659a2feaa0c55ad015a3bf4f1b2b0b822cd15d6c15b0f00a08")
    end)

    test.it("hashes test string correctly", fun ()
        let result = hash.sha256("Hello, Worldnot ")
        test.assert_type(result, "Str")        test.assert_eq(result.len(), 64) # SHA-256 produces 32 bytes = 64 hex chars
    end)
end)
...
test.describe("SHA-512", fun ()
    test.it("hashes empty string correctly", fun ()
        ...
        test.assert_eq(hash.sha512(""), expected)
    end)

    test.it("hashes test string correctly", fun ()
        let result = hash.sha512("Hello, Worldnot ")
        test.assert_type(result, "Str")        test.assert_eq(result.len(), 128) # SHA-512 produces 64 bytes = 128 hex chars
    end)
end)

test.describe("CRC32", fun ()
    test.it("generates checksum for empty string", fun ()
        test.assert_eq(hash.crc32(""), "00000000")
    end)

    test.it("generates checksum for test string", fun ()
        let result = hash.crc32("Hello, Worldnot ")
        test.assert_type(result, "Str")        test.assert_eq(result.len(), 8) # CRC32 produces 4 bytes = 8 hex chars
    end)
