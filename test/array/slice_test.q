...
test.describe("Basic Slicing", fun ()
    ...
    test.it("slices entire array", fun ()
        ...
        test.assert_eq(result.len(), 5, nil)
        test.assert_eq(result[0], 0, nil)
        test.assert_eq(result[4], 4, nil)
    end)
end)
...
test.describe("Negative Indices", fun ()
    test.it("uses negative start index", fun ()
        ...
        test.assert_eq(result.len(), 3, nil)
        test.assert_eq(result[0], 3, nil)
        test.assert_eq(result[2], 5, nil)
    end)
...
test.describe("Edge Cases", fun ()
    ...
    test.it("slices single-element array", fun ()
        ...
        let result = arr.slice(0, 1)
        test.assert_eq(result.len(), 1, nil)
        test.assert_eq(result[0], 42, nil)
    end)
...
test.describe("Practical Use Cases", fun ()
    test.it("extracts head of array", fun ()
        ...
        test.assert_eq(head.len(), 3, nil)
        test.assert_eq(head[0], 1, nil)
        test.assert_eq(head[2], 3, nil)
    end)

    test.it("extracts tail of array", fun ()
        let arr = [1, 2, 3, 4, 5]
        let tail = arr.slice(-3, 5)
        test.assert_eq(tail.len(), 3, nil)
        test.assert_eq(tail[0], 3, nil)
        test.assert_eq(tail[2], 5, nil)
    end)

    test.it("removes first element", fun ()
        let arr = [1, 2, 3, 4, 5]
        let without_first = arr.slice(1, 5)
        test.assert_eq(without_first.len(), 4, nil)
        test.assert_eq(without_first[0], 2, nil)
    end)

    test.it("removes last element", fun ()
        let arr = [1, 2, 3, 4, 5]
        let without_last = arr.slice(0, -1)
        test.assert_eq(without_last.len(), 4, nil)
        test.assert_eq(without_last[3], 4, nil)
    end)

    test.it("removes first and last elements", fun ()
        let arr = [1, 2, 3, 4, 5]
        let middle = arr.slice(1, -1)
        test.assert_eq(middle.len(), 3, nil)
        test.assert_eq(middle[0], 2, nil)
        test.assert_eq(middle[2], 4, nil)
    end)

    test.it("gets middle third of array", fun ()
        let arr = [1, 2, 3, 4, 5, 6, 7, 8, 9]
        let third = arr.slice(3, 6)
        test.assert_eq(third.len(), 3, nil)
        test.assert_eq(third[0], 4, nil)
        test.assert_eq(third[2], 6, nil)
    end)
end)
...
test.describe("Immutability", fun ()
    test.it("does not modify original array", fun ()
        ...
        test.assert_eq(original.len(), 5, nil)
        test.assert_eq(original[0], 1, nil)
        test.assert_eq(original[4], 5, nil)
    end)

    test.it("creates independent copy", fun ()
        let original = [1, 2, 3, 4, 5]
        let sliced = original.slice(1, 4)
        let modified = sliced.push(99)
        test.assert_eq(original.len(), 5, nil)
        test.assert_eq(sliced.len(), 3, nil)
        test.assert_eq(modified.len(), 4, nil)
    end)
end)
...
test.describe("Different Types", fun ()
    test.it("slices string array", fun ()
        ...
        test.assert_eq(result.len(), 2, nil)
        test.assert_eq(result[0], "world", nil)
        test.assert_eq(result[1], "from", nil)
    end)
    ...
    test.it("slices mixed type array", fun ()
        ...
        test.assert_eq(result.len(), 3, nil)
        test.assert_eq(result[0], "two", nil)
        test.assert_eq(result[1], 3, nil)
        test.assert_eq(result[2], "four", nil)
    end)
    ...
    test.it("slices nested arrays", fun ()
        ...
        test.assert_eq(result.len(), 2, nil)
        test.assert_eq(result[0][0], 3, nil)
        test.assert_eq(result[1][1], 6, nil)
    end)
end)
