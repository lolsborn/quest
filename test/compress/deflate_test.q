use "std/test" {it, describe, module, assert_eq, assert_lt, assert_lte, assert_gte, assert_type, fail}
use "std/compress/deflate"

module("std/compress/deflate")

describe("deflate.compress and deflate.decompress", fun ()
  ...
  it("supports different compression levels", fun ()
    ...
    assert_eq(deflate.decompress(fast).decode("utf-8"), data)
    assert_eq(deflate.decompress(default).decode("utf-8"), data)
    assert_eq(deflate.decompress(best).decode("utf-8"), data)
  end)
  ...
  it("validates compression level range", fun ()
    ...
    try
      ...
      fail("Should have raised error for level < 0")
    catch e
  ...
  it("level 0 produces valid deflate", fun ()
    let data = "test data"
    let compressed = deflate.compress(data, 0)
    let decompressed = deflate.decompress(compressed)

    assert_eq(decompressed.decode("utf-8"), data, "Level 0 should work")
    # Level 0 should be larger than compressed (no actual compression)
    assert_gte(compressed.len(), data.len(), "Level 0 adds overhead without compressing")
  end)
end)

describe("deflate error handling", fun ()
  it("rejects invalid compressed data", fun ()
    let invalid = b"not deflate data"
    try
      deflate.decompress(invalid)
      fail("Should have raised error for invalid deflate data")
    catch e
      # Error raised as expected
    end
  end)
