use "std/test" {it, describe, module, assert_eq, assert_lt, assert_lte, assert_gte, assert_type}
use "std/compress/zlib"

module("std/compress/zlib")

describe("zlib.compress and zlib.decompress", fun ()
  ...
  it("supports different compression levels", fun ()
    ...
    assert_eq(zlib.decompress(fast).decode("utf-8"), data)
    assert_eq(zlib.decompress(default).decode("utf-8"), data)
    assert_eq(zlib.decompress(best).decode("utf-8"), data)
  end)
  ...
  it("level 0 produces valid zlib", fun ()
    let data = "test data"
