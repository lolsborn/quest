...
test.describe("zlib.compress and zlib.decompress", fun ()
    ...
    test.it("supports different compression levels", fun ()
        ...
        test.assert_eq(zlib.decompress(fast).decode("utf-8"), data, nil)
        test.assert_eq(zlib.decompress(default).decode("utf-8"), data, nil)
        test.assert_eq(zlib.decompress(best).decode("utf-8"), data, nil)
    end)
