...
test.describe("Malformed hex literals should error", fun ()
    test.it("rejects non-hex digits", fun ()
        ...
        test.assert_raises(SyntaxErr, fun ()
...
test.describe("Valid edge cases that should work", fun ()
    test.it("handles zero in all bases", fun ()
        test.assert_eq(0x0, 0)        test.assert_eq(0b0, 0)        test.assert_eq(0o0, 0)    end)

    test.it("handles single digit in all bases", fun ()
        test.assert_eq(0x1, 1)        test.assert_eq(0b1, 1)        test.assert_eq(0o1, 1)    end)

    test.it("handles uppercase and lowercase prefixes", fun ()
        test.assert_eq(0xFF, 0xff)        test.assert_eq(0XFF, 0xff)        test.assert_eq(0B1111, 0b1111)        test.assert_eq(0O777, 0o777)    end)
end)
