...
test.describe("Hexadecimal literals", fun ()
    ...
    test.it("handles color codes", fun ()
        ...
        test.assert_eq(red, 16711680, nil)
        test.assert_eq(green, 65280, nil)
        test.assert_eq(blue, 255, nil)
        test.assert_eq(white, 16777215, nil)
    end)
end)
...
test.describe("Octal literals", fun ()
    ...
    test.it("handles file permissions", fun ()
        ...
        test.assert_eq(rwxrxrx, 493, nil)
        test.assert_eq(rwr_r_, 420, nil)
        test.assert_eq(rwxrwxrwx, 511, nil)
        test.assert_eq(r__r__r__, 292, nil)
    end)
end)
...
test.describe("Real-world use cases", fun ()
    test.it("bit flags", fun ()
        ...
        test.assert_eq(READ, 1, nil)
        test.assert_eq(WRITE, 2, nil)
        test.assert_eq(EXECUTE, 4, nil)
        test.assert_eq(RW, 3, nil)
        test.assert_eq(RWX, 7, nil)
    end)
    ...
    test.it("bit masking", fun ()
        ...
        test.assert_eq(result, 0b10100000, nil)
    end)
    ...
    test.it("color extraction with bitwise AND", fun ()
        ...
        test.assert_eq(green_channel, 0x008800, nil)
    end)
end)
