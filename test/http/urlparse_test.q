...
test.describe("URL parsing", fun ()
    test.it("parses complete URL with all components", fun ()
        ...
        test.assert_eq(url.get("hostname")._str(), "example.com", "Hostname should be extracted")
        test.assert_eq(url.get("port")._str(), "8080", "Port should be 8080")
        ...
        test.assert_eq(url.get("username")._str(), "user", "Username should be extracted")
    end)
    ...
    test.it("parses simple URL without port or credentials", fun ()
        ...
        test.assert_eq(url.get("hostname")._str(), "example.com", "Hostname parsed")
...
test.describe("Query string parsing", fun ()
    test.it("parses query string into dict of arrays with parse_qs", fun ()
        ...
        test.assert_eq(params.get("name")._str(), "[Alice]", "Name should be array with Alice")
        test.assert_eq(params.get("age")._str(), "[30]", "Age should be array with 30")
        test.assert_eq(params.get("city")._str(), "[NYC]", "City should be array with NYC")
    end)
    ...
    test.it("parses query string into array of pairs with parse_qsl", fun ()
        ...
        test.assert_eq(pairs[0]._str(), "[a, 1]", "First pair should be [a, 1]")
        test.assert_eq(pairs[1]._str(), "[b, 2]", "Second pair should be [b, 2]")
        test.assert_eq(pairs[2]._str(), "[c, 3]", "Third pair should be [c, 3]")
    end)
    ...
    test.it("handles URL-encoded values in query string", fun ()
        ...
        test.assert_eq(params.get("message")._str(), "[Hello World]", "Should decode %20 to space")
        test.assert_eq(params.get("special")._str(), "[a+b]", "Should decode %2B to +")
    end)
end)
