...
test.describe("Logical OR Operator", fun ()
    ...
    test.it("false or false returns false", fun ()
        test.assert(!(false or false), nil)
    end)

    test.it("chains multiple OR operations", fun ()
        test.assert(false or false or true, nil)
        test.assert(!(false or false or false), nil)
    end)
end)

test.describe("Logical NOT Operator", fun ()
    test.it("!true returns false", fun ()
        test.assert(!true == false, nil)
    end)

    test.it("!false returns true", fun ()
        test.assert(!false == true, nil)
    end)

    # NOTE: Double negation !(!x) doesn't parse - grammar limitation
...
test.describe("Combined Logical Operations", fun ()
    test.it("AND has higher precedence than OR", fun ()
        ...
        test.assert(!(false and false or false), nil)
    end)
    ...
    test.it("complex boolean expressions", fun ()
        ...
        test.assert(!((false or false) and true), nil)
    end)
end)
