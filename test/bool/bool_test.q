...
test.describe("Combined Logical Operations", fun ()
    test.it("AND has higher precedence than OR", fun ()
        test.assert(true or false and false)        test.assert(not (false and false or false), nil)
    end)

    test.it("combines AND, OR, and NOT", fun ()
        test.assert(not false and true)        test.assert(not false or false)        test.assert(not (false or false), nil)
    end)
...
test.describe("Comparison Operators - Equality", fun ()
    test.it("compares numbers for equality", fun ()
        test.assert(5 == 5)        test.assert(not (5 == 6), nil)
    end)

    test.it("compares strings for equality", fun ()
        test.assert("hello" == "hello")        test.assert(not ("hello" == "world"), nil)
    end)

    test.it("compares booleans for equality", fun ()
        test.assert(true == true)        test.assert(false == false)        test.assert(not (true == false), nil)
    end)

    test.it("compares numbers for inequality", fun ()
        test.assert(5 != 6)        test.assert(not (5 != 5), nil)
    end)

    test.it("compares strings for inequality", fun ()
        test.assert("hello" != "world")        test.assert(not ("hello" != "hello"), nil)
    end)
end)

test.describe("Comparison Operators - Relational", fun ()
    test.it("less than operator", fun ()
        test.assert(3 < 5)        test.assert(not (5 < 3), nil)
        test.assert(not (5 < 5), nil)
    end)

    test.it("less than or equal operator", fun ()
        test.assert(3 <= 5)        test.assert(5 <= 5)        test.assert(not (5 <= 3), nil)
    end)

    test.it("greater than operator", fun ()
        test.assert(5 > 3)        test.assert(not (3 > 5), nil)
        test.assert(not (5 > 5), nil)
    end)

    test.it("greater than or equal operator", fun ()
        test.assert(5 >= 3)        test.assert(5 >= 5)        test.assert(not (3 >= 5), nil)
    end)
...
test.describe("String Comparison", fun ()
    ...
    test.it("handles string equality with case sensitivity", fun ()
        test.assert("Hello" == "Hello")        test.assert(not ("Hello" == "hello"), nil)
    end)
