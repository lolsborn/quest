...
test.describe("calcsize", fun ()
    test.it("calculates size for simple formats", fun ()
        test.assert_eq(struct.calcsize("b"), 1, nil)
        test.assert_eq(struct.calcsize("B"), 1, nil)
        test.assert_eq(struct.calcsize("h"), 2, nil)
        test.assert_eq(struct.calcsize("H"), 2, nil)
        test.assert_eq(struct.calcsize("i"), 4, nil)
        test.assert_eq(struct.calcsize("I"), 4, nil)
        test.assert_eq(struct.calcsize("q"), 8, nil)
        test.assert_eq(struct.calcsize("Q"), 8, nil)
        test.assert_eq(struct.calcsize("f"), 4, nil)
        test.assert_eq(struct.calcsize("d"), 8, nil)
    end)

    test.it("calculates size for repeated formats", fun ()
        test.assert_eq(struct.calcsize("3i"), 12, nil)
        test.assert_eq(struct.calcsize("2H"), 4, nil)
        test.assert_eq(struct.calcsize("4b"), 4, nil)
    end)

    test.it("calculates size for mixed formats", fun ()
        test.assert_eq(struct.calcsize("HHI"), 8, nil)
        test.assert_eq(struct.calcsize("bHi"), 7, nil)
    end)
    ...
    test.it("calculates size for strings", fun ()
        test.assert_eq(struct.calcsize("10s"), 10, nil)
        test.assert_eq(struct.calcsize("5s"), 5, nil)
    end)
end)
...
test.describe("pack and unpack", fun ()
    ...
    test.it("packs and unpacks multiple values", fun ()
        ...
        test.assert_eq(data.len(), 8, nil)

        let values = struct.unpack("<HHI", data)
        test.assert_eq(values.len(), 3, nil)
        test.assert_eq(values[0], 1, nil)
        test.assert_eq(values[1], 2, nil)
        test.assert_eq(values[2], 3, nil)
    end)

    test.it("handles little-endian byte order", fun ()
        let data = struct.pack("<H", 258)  # 0x0102
        test.assert_eq(data.len(), 2, nil)
        test.assert_eq(data.get(0), 2, nil)   # Little-endian: low byte first
        test.assert_eq(data.get(1), 1, nil)   # High byte second
    end)

    test.it("handles big-endian byte order", fun ()
        let data = struct.pack(">H", 258)  # 0x0102
        test.assert_eq(data.len(), 2, nil)
        test.assert_eq(data.get(0), 1, nil)   # Big-endian: high byte first
        test.assert_eq(data.get(1), 2, nil)   # Low byte second
    end)
...
test.describe("booleans", fun ()
    test.it("handles boolean values", fun ()
        ...
        test.assert_eq(values[0], true, nil)
        test.assert_eq(values[1], false, nil)
    end)
end)
...
test.describe("strings", fun ()
    test.it("handles fixed-length strings", fun ()
        ...
        test.assert_eq(data.len(), 10, nil)

        let values = struct.unpack("10s", data)
        test.assert_eq(values[0], "Hello", nil)  # Null bytes are stripped
    end)

    test.it("truncates strings that are too long", fun ()
        let data = struct.pack("5s", "HelloWorld")
        test.assert_eq(data.len(), 5, nil)

        let values = struct.unpack("5s", data)
        test.assert_eq(values[0], "Hello", nil)
    end)
end)
...
test.describe("chars", fun ()
    test.it("handles single characters", fun ()
        ...
        test.assert_eq(values[0], "A", nil)
        test.assert_eq(values[1], "B", nil)
        test.assert_eq(values[2], "C", nil)
    end)
end)
...
test.describe("pad bytes", fun ()
    test.it("handles pad bytes", fun ()
        ...
        test.assert_eq(data.len(), 7, nil)  # 4 (int) + 1 (pad) + 2 (short)

        let values = struct.unpack("ixH", data)
        test.assert_eq(values.len(), 2, nil)
        test.assert_eq(values[0], 42, nil)
        test.assert_eq(values[1], 100, nil)
    end)
end)
...
test.describe("repeated values", fun ()
    test.it("handles repeated format characters", fun ()
        ...
        test.assert_eq(data.len(), 12, nil)
...
test.describe("unpack_from", fun ()
    test.it("unpacks from offset", fun ()
        ...
        test.assert_eq(data.len(), 6, nil)

        # Unpack from offset 2 (skip first short)
        let values = struct.unpack_from("<HH", data, 2)
        test.assert_eq(values.len(), 2, nil)
        test.assert_eq(values[0], 200, nil)
        test.assert_eq(values[1], 300, nil)
    end)
end)
...
test.describe("real-world use cases", fun ()
    test.it("handles network packet header", fun ()
        ...
        test.assert_eq(values[0], 1, nil)      # version
        test.assert_eq(values[1], 5, nil)      # type
        test.assert_eq(values[2], 1024, nil)   # length
        test.assert_eq(values[3], 12345, nil)  # id
    end)
    ...
    test.it("handles binary file format", fun ()
        ...
        test.assert_eq(data.len(), 16, nil)

        let values = struct.unpack("<4sHHQ", data)
        test.assert_eq(values[0], "ABCD", nil)
        test.assert_eq(values[1], 100, nil)
        test.assert_eq(values[2], 7, nil)
        test.assert_eq(values[3], 1234567890, nil)
    end)
end)
