...
test.describe("os.environ edge cases", fun ()
    test.it("environ does not accept arguments", fun ()
        try
            ...
            test.assert(e.message().contains("expects 0 arguments"))
        end
    end)
end)
...
test.describe("os environment integration", fun ()
    test.it("can set, get, and unset multiple vars", fun ()
        ...
        test.assert_eq(os.getenv("VAR1"), "value1")
        test.assert_eq(os.getenv("VAR2"), "value2")
        test.assert_eq(os.getenv("VAR3"), "value3")

        os.unsetenv("VAR1")
        os.unsetenv("VAR2")
        os.unsetenv("VAR3")

        test.assert_eq(os.getenv("VAR1"), nil)
        test.assert_eq(os.getenv("VAR2"), nil)
        test.assert_eq(os.getenv("VAR3"), nil)
    end)
end)
