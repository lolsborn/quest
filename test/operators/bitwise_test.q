...
test.describe("Bitwise OR (|)", fun ()
    ...
    test.it("handles zero", fun ()
        test.assert_eq(0 | 0, 0)        test.assert_eq(42 | 0, 42)        test.assert_eq(0 | 42, 42)    end)

    test.it("is commutative", fun ()
        test.assert_eq(5 | 3, 3 | 5)        test.assert_eq(0xFF | 0x0F, 0x0F | 0xFF)    end)
end)
...
test.describe("Bitwise AND (&)", fun ()
    ...
    test.it("handles zero", fun ()
        test.assert_eq(0 & 0, 0)        test.assert_eq(42 & 0, 0, "AND with 0 gives 0")
    ...
    test.it("masks bits efficiently", fun ()
        test.assert_eq(255 & 0xFF, 255)        test.assert_eq(256 & 0xFF, 0, "256 mod 256 via masking")
...
test.describe("Bitwise XOR (^)", fun ()
    ...
    test.it("handles zero", fun ()
        test.assert_eq(0 ^ 0, 0)        test.assert_eq(42 ^ 0, 42, "XOR with 0 gives original")
        test.assert_eq(0 ^ 42, 42)    end)
...
test.describe("Left shift (<<)", fun ()
    test.it("shifts bits left", fun ()
        test.assert_eq(1 << 0, 1)        test.assert_eq(1 << 1, 2)        test.assert_eq(1 << 8, 256)        test.assert_eq(1 << 16, 65536)    end)
    ...
    test.it("handles zero shift", fun ()
        test.assert_eq(42 << 0, 42)    end)
end)

test.describe("Right shift (>>)", fun ()
    test.it("shifts bits right", fun ()
        test.assert_eq(256 >> 0, 256)        test.assert_eq(256 >> 1, 128)        test.assert_eq(256 >> 8, 1)        test.assert_eq(255 >> 1, 127)    end)
