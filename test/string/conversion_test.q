...
test.describe("to_int() - basic conversions", fun ()
    test.it("converts decimal string to int", fun ()
        test.assert_eq("123".to_int(), 123, nil)
        test.assert_eq("0".to_int(), 0, nil)
        test.assert_eq("-456".to_int(), -456, nil)
    end)
    ...
    test.it("handles underscores in numbers", fun ()
        test.assert_eq("1_000_000".to_int(), 1000000, nil)
        test.assert_eq("123_456".to_int(), 123456, nil)
    end)

    test.it("converts hexadecimal strings", fun ()
        test.assert_eq("0xFF".to_int(), 255, nil)
        test.assert_eq("0x10".to_int(), 16, nil)
        test.assert_eq("0xDEADBEEF".to_int(), 3735928559, nil)
    end)

    test.it("converts binary strings", fun ()
        test.assert_eq("0b1010".to_int(), 10, nil)
        test.assert_eq("0b11111111".to_int(), 255, nil)
        test.assert_eq("0b0".to_int(), 0, nil)
    end)

    test.it("converts octal strings", fun ()
        test.assert_eq("0o755".to_int(), 493, nil)
        test.assert_eq("0o10".to_int(), 8, nil)
...
test.describe("to_float() - basic conversions", fun ()
    test.it("converts decimal string to float", fun ()
        test.assert_eq("3.14".to_float(), 3.14, nil)
        test.assert_eq("0.0".to_float(), 0.0, nil)
        test.assert_eq("-2.5".to_float(), -2.5, nil)
    end)

    test.it("converts integer strings to float", fun ()
        test.assert_eq("42".to_float(), 42.0, nil)
        test.assert_eq("-10".to_float(), -10.0, nil)
    end)

    test.it("handles scientific notation", fun ()
        test.assert_eq("1e3".to_float(), 1000.0, nil)
        test.assert_eq("1.5e2".to_float(), 150.0, nil)
        test.assert_eq("2.5e-2".to_float(), 0.025, nil)
    end)
...
test.describe("Cross-type conversions", fun ()
    test.it("int string converts to all types", fun ()
        let s = "42"
        test.assert_eq(s.to_int(), 42, nil)
        test.assert_eq(s.to_float(), 42.0, nil)
        test.assert_eq(s.to_decimal().to_string(), "42", nil)
        test.assert_eq(s.to_bigint().to_string(), "42", nil)
    end)

    test.it("negative numbers work across types", fun ()
        let s = "-100"
        test.assert_eq(s.to_int(), -100, nil)
        test.assert_eq(s.to_float(), -100.0, nil)
        test.assert_eq(s.to_decimal().to_string(), "-100", nil)
        test.assert_eq(s.to_bigint().to_string(), "-100", nil)
    end)

    test.it("zero works across types", fun ()
        let s = "0"
        test.assert_eq(s.to_int(), 0, nil)
        test.assert_eq(s.to_float(), 0.0, nil)
        test.assert_eq(s.to_decimal().to_string(), "0", nil)
        test.assert_eq(s.to_bigint().to_string(), "0", nil)
    end)
end)
...
test.describe("Real-world use cases", fun ()
    ...
    test.it("parses price with decimal", fun ()
        ...
        test.assert_eq(price.to_string(), "19.99", nil)
    end)

    test.it("parses large ID as bigint", fun ()
        let id_str = "1234567890123456789"
        let id = id_str.to_bigint()
        test.assert_eq(id.to_string(), "1234567890123456789", nil)
    end)
end)
