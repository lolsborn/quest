...
test.describe("split()", fun ()
    ...
    test.it("splits by multi-character delimiter", fun ()
        ...
        test.assert_eq(result.len(), 3)
        test.assert_eq(result[0], "one")        test.assert_eq(result[1], "two")        test.assert_eq(result[2], "three")    end)

    test.it("splits path by forward slash", fun ()
        let path = "/foo/bar/test.q"
        let parts = path.split("/")
        test.assert_eq(parts.len(), 4)
        test.assert_eq(parts[0], "")        test.assert_eq(parts[1], "foo")        test.assert_eq(parts[2], "bar")        test.assert_eq(parts[3], "test.q")    end)
    ...
    test.it("handles no delimiter found", fun ()
        ...
        test.assert_eq(result.len(), 1)
        test.assert_eq(result[0], "hello")    end)
    ...
    test.it("handles consecutive delimiters", fun ()
        ...
        test.assert_eq(result.len(), 3)
        test.assert_eq(result[0], "a")        test.assert_eq(result[1], "")        test.assert_eq(result[2], "b")    end)
end)
...
test.describe("slice()", fun ()
    ...
    test.it("extracts substring from middle", fun ()
        ...
        test.assert_eq(result, "World")    end)
