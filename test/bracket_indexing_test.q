...
test.describe("String bracket indexing", fun ()
    test.it("returns single character", fun ()
        test.assert_eq("hello"[0], "h", nil)
        test.assert_eq("hello"[1], "e", nil)
        test.assert_eq("hello"[2], "l", nil)
        test.assert_eq("hello"[3], "l", nil)
        test.assert_eq("hello"[4], "o", nil)
    end)

    test.it("supports negative indexing", fun ()
        test.assert_eq("hello"[-1], "o", nil)
        test.assert_eq("hello"[-2], "l", nil)
        test.assert_eq("hello"[-3], "l", nil)
        test.assert_eq("hello"[-4], "e", nil)
        test.assert_eq("hello"[-5], "h", nil)
    end)
    ...
    test.it("handles UTF-8 code points correctly", fun ()
        ...
        test.assert_eq(s[5], "世", nil)
        test.assert_eq(s[6], "界", nil)
    end)
    ...
    test.it("works with single character strings", fun ()
        test.assert_eq("a"[0], "a", nil)
        test.assert_eq("a"[-1], "a", nil)
    end)

    test.it("works with multi-line strings", fun ()
        let s = "hello\nworld"
        test.assert_eq(s[5], "\n", nil)
        test.assert_eq(s[6], "w", nil)
    end)
end)
...
test.describe("Bytes bracket indexing", fun ()
    ...
    test.it("handles boundary indices", fun ()
        test.assert_eq(b"hello"[0], 104, nil)
        test.assert_eq(b"hello"[4], 111, nil)
        test.assert_eq(b"hello"[-1], 111, nil)
        test.assert_eq(b"hello"[-5], 104, nil)
    end)
    ...
    test.it("handles full byte range (0-255)", fun ()
        let data = b"\x00\xFF"
        test.assert_eq(data[0], 0, nil)
        test.assert_eq(data[1], 255, nil)
    end)
    ...
    test.it("works with single byte", fun ()
        test.assert_eq(b"a"[0], 97, nil)
        test.assert_eq(b"a"[-1], 97, nil)
    end)
end)
