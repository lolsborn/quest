...
test.describe("String bracket indexing", fun ()
  test.it("returns single character", fun ()
    ...
    test.assert_eq("hello"[1], "e")
    test.assert_eq("hello"[2], "l")
    test.assert_eq("hello"[3], "l")
  ...
  test.it("handles UTF-8 code points correctly", fun ()
    ...
    test.assert_eq(s[5], "世")
    test.assert_eq(s[6], "界")
  end)
  ...
  test.it("works with single character strings", fun ()
    test.assert_eq("a"[0], "a")
    test.assert_eq("a"[-1], "a")
  end)

  test.it("works with multi-line strings", fun ()
    let s = "hello\nworld"
    test.assert_eq(s[5], "\n")
    test.assert_eq(s[6], "w")
  end)
end)
...
test.describe("Bytes bracket indexing", fun ()
  ...
  test.it("handles full byte range (0-255)", fun ()
    let data = b"\x00\xFF"
    test.assert_eq(data[0], 0)
    test.assert_eq(data[1], 255)
  end)
  ...
  test.it("works with single byte", fun ()
    test.assert_eq(b"a"[0], 97)
    test.assert_eq(b"a"[-1], 97)
  end)
end)
