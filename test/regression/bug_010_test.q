...
test.module("Bug 010: Mutable Type Fields")
...
test.describe("Mutable struct fields", fun ()
    test.it("increments field value", fun ()
        ...
        test.assert_eq(c.count, 0, "Initial count should be 0")

        let result = c.increment()
        test.assert_eq(result, 1, "Method should return 1")
        test.assert_eq(c.count, 1, "Field should be updated to 1")
    end)
    ...
    test.it("persists mutations across multiple calls", fun ()
        ...
        c.increment()
        c.increment()
        test.assert_eq(c.count, 3, "Count should be 3 after three increments")
    end)
    ...
    test.it("maintains separate state for multiple instances", fun ()
        ...
        test.assert_eq(c1.count, 2, "c1 should be 2")
        test.assert_eq(c2.count, 1, "c2 should be 1")
    end)
    ...
    test.it("works with methods that return values", fun ()
        ...
        test.assert_eq(p.next_char(), "h", "First char should be h")
        test.assert_eq(p.pos, 1, "Position should advance to 1")
        test.assert_eq(p.next_char(), "e", "Second char should be e")
        test.assert_eq(p.pos, 2, "Position should advance to 2")
    end)
    ...
    test.it("works with compound assignment operators", fun ()
        ...
        v.add(5)
        test.assert_eq(v.num, 15, "After add(5)")

        v.multiply(2)
        test.assert_eq(v.num, 30, "After multiply(2)")
    end)
end)
