...
module("Self Corruption Bug Regression")

describe("Self isolation in nested method calls", fun ()
  it("self stays correct when calling methods on objects from self.field", fun ()
    ...
    assert_eq(result, "TestContainer: 3 handlers processed")
  end)

  it("self is not affected by nested self in inner method", fun ()
    type Inner
      value: Int
    ...
    assert_eq(result, 25)  # 5 + (10 * 2) = 25
  end)

  it("self in loops with array of objects", fun ()
    type Item
      name: Str
      fun get_name()
        return self.name
    ...
    assert(coll.process_all(), "Collection count should match items processed")
  end)
end)

describe("Self in deeply nested calls", fun ()
  it("maintains correct self through multiple nesting levels", fun ()
    type Level3
      name: Str
      fun identify()
        return "L3:" .. self.name
    ...
    assert_eq(result, "L1:top/L2:middle/L3:deep")
  end)
end)
