...
test.module("Bug 021: Return at Top Level of Script")

# ----------------------------------------------------------------------------
# Test 1: Basic top-level return
# ----------------------------------------------------------------------------
test.describe("Basic top-level return", fun ()
    test.it("executes code before return", fun ()
    ...
    test.it("return exits before subsequent code", fun ()
    ...
    test.it("bare return exits cleanly without error", fun ()
...
# ----------------------------------------------------------------------------
test.describe("Return with value at top level", fun ()
    test.it("return with value exits cleanly", fun ()
    ...
    test.it("return with complex expression", fun ()
...
# ----------------------------------------------------------------------------
test.describe("Function returns still work correctly", fun ()
    test.it("return inside function works normally", fun ()
    ...
    test.it("multiple returns in function", fun ()
        let code = """
fun get_status(x)
    if x < 0
        return "negative"
...
"""
        let result = sys.eval(code)
        test.assert_eq(result, "negative,zero,positive")
    end)

    test.it("function return vs top-level return", fun ()
...
# ----------------------------------------------------------------------------
test.describe("Return in control flow at top level", fun ()
    test.it("return inside if statement", fun ()
        let code = """
let x = 10
if x > 5
    return
...
"""
    ...
    test.it("return inside while loop", fun ()
        let code = """
let count = 0
while count < 10
    count = count + 1
...
"""
    ...
    test.it("return inside for loop", fun ()
        let code = """
let sum = 0
for i in [1, 2, 3, 4, 5]
    sum = sum + i
...
# ----------------------------------------------------------------------------
test.describe("Return followed by newline", fun ()
    test.it("bare return followed by newline doesn't consume next statement", fun ()
    ...
    test.it("return with value on same line", fun ()
        let code = "let x = 42\nreturn x\nlet y = 999"
        let result = sys.eval(code)
        test.assert_nil(result)
    end)
...
# ----------------------------------------------------------------------------
test.describe("Multiple returns", fun ()
    test.it("only first return executes", fun ()
        let code = """
let x = 1
return
let y = 2
...
"""
    ...
    test.it("return in if blocks - first match exits", fun ()
        let code = """
let status = "active"
if status == "active"
    return
...
# ----------------------------------------------------------------------------
test.describe("Return interaction with try/catch", fun ()
    test.it("return inside try block", fun ()
        let code = """
try
    let x = 10
    return
...
"""
    ...
    test.it("return inside catch block", fun ()
        let code = """
try
    raise "test error"
catch e
...
# ----------------------------------------------------------------------------
test.describe("Return vs sys.exit()", fun ()
    test.it("return is cleaner than sys.exit() for script exit", fun ()
