# std/web/middleware/router.q - Flexible Path Parameter Routing (QEP-062)
#
...
let _default_router = {routes: []}  # Router state: array of routes

# =============================================================================
# Module-Level Functions (Default Router API)
# =============================================================================

# Register GET route on default router
pub fun get(pattern, handler)
  register_route("GET", pattern, handler)
end

# Register POST route on default router
pub fun post(pattern, handler)
  register_route("POST", pattern, handler)
end

# Register PUT route on default router
pub fun put(pattern, handler)
  register_route("PUT", pattern, handler)
end

# Register DELETE route on default router
pub fun delete(pattern, handler)
  register_route("DELETE", pattern, handler)
end

# Register PATCH route on default router
pub fun patch(pattern, handler)
  register_route("PATCH", pattern, handler)
end

# Internal: Register a route on default router
fun register_route(method, pattern, handler)
  let segments = parse_pattern(pattern)
  let priority = calculate_priority(segments)
  ...
  _default_router["routes"].push(route)
  sort_routes(_default_router["routes"])
end
...
pub fun dispatch_middleware(req)
  let path = req["path"]
  let method = req["method"]
  let routes = _default_router["routes"]
